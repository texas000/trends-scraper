...
async function getTrendingSearches(countryCode = 'KR') {
  ...
  try {
    ...
    // 트렌드를 찾지 못한 경우 - 경고만 하고 계속 진행
    if (trends.length === 0) {
      console.warn(`⚠️ ${countryInfo.name}에서 실시간 트렌드를 파싱하지 못함`);
      // Fallback: 링크에서 텍스트 직접 추출
      const fallbackTrends = await page.evaluate(() => {
        ...
        for (let el of links) {
          ...
          const text = el.textContent?.substring(0, 150).trim();
          if (text && text.length > 2 && text.length < 150 && !seen.has(text)) {
            seen.add(text);
            keywords.push({ keyword: text, rank: keywords.length + 1, breakdown_text: null });
          }
        }
        return keywords;
      }).catch(err => {

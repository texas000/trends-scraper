...
async function initializeDatabase() {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_country_search ON search_results(country_code, searched_at DESC);
    `);
    ...
    // 마이그레이션: search_results 테이블에 country_code 열이 없으면 추가
    try {
      await client.query(`
        ALTER TABLE search_results
        ADD COLUMN country_code VARCHAR(5);
      `);
      console.log('✓ country_code 컬럼 추가 완료');
    } catch (e) {
      // 이미 존재하면 무시
      if (e.message.includes('already exists')) {
        console.log('✓ country_code 컬럼이 이미 존재합니다');
      } else {
        throw e;
      }
    }

    // 마이그레이션: search_results 테이블에 articles 열이 없으면 추가
    try {
      await client.query(`
        ALTER TABLE search_results
        ADD COLUMN articles JSONB;
      `);
      console.log('✓ articles 컬럼 추가 완료');
    } catch (e) {
      // 이미 존재하면 무시
      if (e.message.includes('already exists')) {
        console.log('✓ articles 컬럼이 이미 존재합니다');
      } else {
        throw e;
      }
    }

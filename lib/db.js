...
async function initializeDatabase() {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_saved_at ON trends(saved_at DESC);
      CREATE INDEX IF NOT EXISTS idx_country_code ON trends(country_code);
      CREATE INDEX IF NOT EXISTS idx_keyword ON trends(keyword);
      CREATE INDEX IF NOT EXISTS idx_country_date ON trends(country_code, saved_at DESC);
    `);

    // DuckDuckGo 검색 결과 테이블
    await client.query(`
      CREATE TABLE IF NOT EXISTS search_results (
        ...
        abstract TEXT,
        abstract_source VARCHAR(255),
        abstract_url VARCHAR(2048),
        image VARCHAR(2048),
        heading VARCHAR(255),
      ...
      CREATE INDEX IF NOT EXISTS idx_keyword_search ON search_results(keyword);
      CREATE INDEX IF NOT EXISTS idx_searched_at ON search_results(searched_at DESC);
      CREATE INDEX IF NOT EXISTS idx_keyword_searched ON search_results(keyword, searched_at DESC);
    `);
    ...
    await client.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_trend_search ON trend_searches(trend_id);
      CREATE INDEX IF NOT EXISTS idx_search_trend ON trend_searches(search_id);
    `);
  ...
  try {
    ...
    for (const trend of trendsData) {
      const result = await client.query(
        `INSERT INTO trends (country_code, country_name, language, keyword, rank, trend_value, saved_at) 
    ...
    for (let i = 0; i < searchResults.length; i++) {
      ...
      const queryResult = await client.query(
        `INSERT INTO search_results 
         (keyword, success, abstract, abstract_source, abstract_url, image, heading, result_count, top_results, related_searches, error, searched_at) 
        ...
        [
          ...
          result.abstract_source || null,
          result.abstract_url || null,
          result.image || null,
          result.heading || null,
          result.result_count || 0,
          result.top_results ? JSON.stringify(result.top_results) : null,
          result.related_searches ? JSON.stringify(result.related_searches) : null,
          result.error || null,
          new Date()
        ]
      );
...
async function getSearchResultsByKeyword(keyword, limit = 10) {
  ...
  try {
    const result = await client.query(`
      SELECT 
        id, keyword, success, abstract, abstract_source, abstract_url, 
        image, heading, result_count, top_results, related_searches, error, searched_at
      FROM search_results
      WHERE keyword = $1
    ...
    return result.rows.map(row => ({
      ...row,
      top_results: row.top_results ? JSON.parse(row.top_results) : [],
      related_searches: row.related_searches ? JSON.parse(row.related_searches) : []
    }));
  } catch (error) {

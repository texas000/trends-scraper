...
async function initializeDatabase() {
  ...
  try {
    ...
    // 2. Create indices for trends table
    ...
    // 4. Migrate search_results: add country_code if missing
    try {
      await client.query(`ALTER TABLE search_results ADD COLUMN country_code VARCHAR(5)`);
    ...
    // 5. Migrate search_results: add articles if missing
    try {
      await client.query(`ALTER TABLE search_results ADD COLUMN articles JSONB`);
